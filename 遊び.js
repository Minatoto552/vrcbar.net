...
function setSelectedUser(name, icon = "") {
  ...
  if ($("globalUserSelect")) {
    $("globalUserSelect").value = selectedUserName;
  }
  ...
  if ($("globalUserIcon")) {
    if (selectedUserIcon) {
      $("globalUserIcon").src = selectedUserIcon;
      $("globalUserIcon").style.display = "block";
    } else {
      $("globalUserIcon").style.display = "none";
    }
  }
}
...
function showAnnouncement(data) {
  if (!$("announcementText") || !$("announcementOverlay")) return;
...
function showAlarm(data) {
  if (!$("alarmText") || !$("alarmOverlay")) return;
...
document.querySelectorAll(".navOrder").forEach(btn => {
  btn.onclick = async () => openPage("order", renderOrderPage);
});

document.querySelectorAll(".navReceive").forEach(btn => {
  btn.onclick = async () => openPage("receive", renderReceivePage);
});

document.querySelectorAll(".navHistory").forEach(btn => {
  btn.onclick = async () => openPage("history", renderHistoryPage);
});
...
$("globalUserSelect").onchange = () => {
  const select = $("globalUserSelect");
  const option = select.options[select.selectedIndex];
  const icon = option?.dataset?.icon || "";
  setSelectedUser(select.value, icon);
};
...
async function renderNameSelects() {
  ...
  const savedRaw = localStorage.getItem(getUserStorageKey());

  if (savedRaw) {
    try {
      const saved = JSON.parse(savedRaw);
      setSelectedUser(saved.name || "", saved.icon || "");
    } catch {
      setSelectedUser("", "");
    }
  } else {
...
$("addOrderNameButton").onclick = async () => {
  ...
  const icon = $("orderIconInput").value.trim();
...
async function renderEmergencyForm() {
  ...
  if (selectedUserName && $("emergencyNameSelect")) {
    $("emergencyNameSelect").value = selectedUserName;
  }
}
  ...
  $("orderHistoryList").innerHTML = orders.map(order => {
    ...
    return `
      <div class="order-card ${esc(order.status)}">
        ...
        <div class="actions">
          <button class="history-status-button status-pending" onclick="setOrderStatusFromHistory('${order.id}', 'pending')">
            未対応に戻す
          </button>

          <button class="history-status-button status-making" onclick="setOrderStatusFromHistory('${order.id}', 'making')">
            対応中にする
          </button>

          <button class="history-status-button status-done" onclick="setOrderStatusFromHistory('${order.id}', 'done')">
            お届け済みにする
          </button>
        </div>
      </div>
